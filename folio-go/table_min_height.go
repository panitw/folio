...
// (folio_expr_validate.go). So this is ParseTemplate's, exactly as those
// are.
//
...
func validateTableMinHeights(t *Template) error {
	...
	if el, minHeight, ok := tableFloorAbove(t, window); ok {
		return newRenderError(DiagCodeTableMinHeightUnplaceable, string(el.ID), "",
			fmt.Errorf("folio: element %s: minHeight is taller than the content window (%spt against a content height of %spt), so the table fits on no page — reduce minHeight, or increase the page's content height by reducing its margins or its page-header/page-footer heights",
