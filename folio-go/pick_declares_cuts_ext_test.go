...
// `fonts` imports `folio`; inside `package folio` that is an import cycle. It
...
func applyAll(t *testing.T, tpl *folio.Template, commands ...string) {
	t.Helper()
	for _, command := range commands {
		if _, err := folio.ApplyComponentCommand(tpl, []byte(command)); err != nil {
...
func documentChains(t *testing.T, tpl *folio.Template) map[string][]json.RawMessage {
	t.Helper()
	out, err := folio.SerializeTemplate(tpl)
	if err != nil {
	...
	if _, err := folio.ParseTemplate(out); err != nil {
...
func TestAPickOfAShippedFamilyDeclaresThatFamilysCutsInTheDocument(t *testing.T) {
	tpl, err := folio.ParseTemplate([]byte(pickBaseDocument(`"Hamburgefonstiv"`)))
	if err != nil {
	...
	res, err := folio.Render(tpl, folio.Data(`{}`), folio.Params(`{}`), fonts.Shipped())
	if err != nil {
		t.Fatalf("render the picked document: %v", err)
	}
	for _, d := range res.Diagnostics {
		if d.Code == folio.DiagCodeTextStyleFaceUndeclared {
...
func TestAPickOfNotoSansSCDeclaresNoCutAndThatIsORDINARY(t *testing.T) {
	tpl, err := folio.ParseTemplate([]byte(pickBaseDocument(`"中文"`)))
	if err != nil {
		t.Fatalf("parse the base document: %v", err)
	}
...
func savedVersion(t *testing.T, tpl *folio.Template) string {
	t.Helper()
	out, err := folio.SerializeTemplate(tpl)
	if err != nil {
		t.Fatal(err)
