...
func TestQRCodeNonSquareBoxIsSizedToTheShortSideAndCentred(t *testing.T) {
	tpl, err := ParseTemplate([]byte(qrcodeTestTemplate(t, "Folio", "", "200", "80", "")))
	if err != nil {
		t.Fatalf("parse: %v", err)
	}
	pages, diags := barcodePages(t, tpl, `{}`)
	...
	want, _ := barcode.EncodeQR([]byte("Folio"), barcode.ECLevelM)
...
func TestQRCodeLoadRefusals(t *testing.T) {
	...
	for _, bad := range []string{`"errorCorrection": "X", `, `"errorCorrection": null, `, `"style": {}, `} {
		if _, err := ParseTemplate([]byte(qrcodeTestTemplate(t, "Folio", "", "120", "120", bad))); err == nil {
...
func TestQRCodeWarnings(t *testing.T) {
	for _, c := range []struct {
		...
		{"module below 0.5 mm", "Folio", "M", "30", `{}`, DiagCodeQRCodeModuleTooSmall, true},
		{"cannot fit", "Folio", "M", "0.02", `{}`, DiagCodeQRCodeDoesNotFit, false},
	} {
		t.Run(c.name, func(t *testing.T) {
...
func TestUnfitStaticQRCodeProjectsDoesNotFitAndNoColumnItem(t *testing.T) {
	for _, c := range []struct{ value, side, reason string }{
		{"Folio", "0.02", qrcodeUnavailableDoesNotFit},
	} {
...
func TestPageHeaderQRCodeRendersOnEveryPage(t *testing.T) {
  ...
  "bands": {
    ...
    "pageFooter": {"elements": [], "height": 20},
    "pageHeader": {"elements": [
      {"id": "e1", "type": "qrcode", "x": 0, "y": 0, "width": 58, "height": 58, "errorCorrection": "L", "value": "Folio"}
    ], "height": 60}
  },
  "fonts": {},
  "locale": "en",
...
`
	...
	want, _ := barcode.EncodeQR([]byte("Folio"), barcode.ECLevelL)
