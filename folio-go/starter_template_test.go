...
// `fonts` imports `folio` — inside `package folio` that is an import cycle.
...
func loadStarterTemplate(t *testing.T) ([]byte, *folio.Template) {
	t.Helper()
	raw, err := os.ReadFile(starterTemplatePath)
	if err != nil {
		t.Fatalf("read the shipped starter template: %v", err)
	}
	tpl, err := folio.ParseTemplate(raw)
	if err != nil {
...
func TestTheShippedStarterDeclaresOnlyCutsTheEngineSupplies(t *testing.T) {
	...
	var chain *folio.CanvasFontChain
...
func TestANewDocumentFromTheStarterCanActuallyBold(t *testing.T) {
	_, tpl := loadStarterTemplate(t)
	if _, err := folio.ApplyComponentCommand(tpl, []byte(`{"kind":"createComponent","version":1,"type":"text","band":"content","x":40,"y":40,"width":200,"height":24,"snap":false}`)); err != nil {
	...
	if _, err := folio.ApplyComponentCommand(tpl, []byte(set)); err != nil {
...
func paintedFaces(projection folio.CanvasProjection) []string {
	var faces []string
