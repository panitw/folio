...
var declaredEpic2GateObligations = []string{
	...
	"matrix-document: embedded-font",          // Story 8.3 (FR53/FR56) — THE FIRST cross-target artifact that CARRIES a font face rather than naming one, and the first declaring format version 2.0 for a reason other than align: "justify". It is registered for a NEGATIVE property, which is unusual and is the point: this story does not render from the embedded face (Story 8.4 does), so the four legs certify that ~47 KB of carried font reaches the LOADER on every target and reaches the PAGE on none of them (matrix_test.go's requireEmbeddedFaceStaysOffThePage). It ships NO expected.pdf, on the hidden-image precedent — an expected.pdf is a human-attested artifact (AD-21/D-4.7.1) and this story cannot produce the one that matters — so goldenDigestRecord stays at 22. ALL FOUR legs were RUN in-story — more than D-000.54's native-leg requirement — and all four targets produce sha256 db400698…e513ad at 55,513 bytes, with TestCrossTargetByteIdentity agreeing; the question the legs answer is this document's whole point (does ~47 KB of carried asset survive four toolchains identically), so deferring them to the Epic 8 gate would have deferred the only thing worth measuring
