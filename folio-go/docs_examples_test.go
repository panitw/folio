...
// folio, fonts and wasm packages must be named in the guide.
...
// two programs from a consumer module that replaces folio-go with this
...
func TestDocsGuideProgramsRunAgainstTheWorkingTree(t *testing.T) {
	...
	for _, program := range []string{"first-pdf", "render-to"} {
		t.Run(program, func(t *testing.T) {
			work := t.TempDir()
			gomod := "module example.com/folio-demo\n\ngo 1.25.0\n\nrequire github.com/panitw/folio/folio-go v0.0.0\n\nreplace github.com/panitw/folio/folio-go => " + filepath.Join(root, "folio-go") + "\n"
			...
			case "render-to":
				for _, want := range []string{"validate: 0 warnings", "bytes equal: true", "failing writer: folio: RenderTo: write failed after 0 of", "is a RenderError: false"} {
