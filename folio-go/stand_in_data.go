...
// TestFolioMethodNamesAreInjective refuses a second receiver in a folio
...
// rather than by discipline — folio-go rendering the same template with
...
func StandInData(tpl *Template) ([]byte, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("folio: stand-in data: %w", err)
	}
...
func (g *standInGenerator) collectElement(tpl *Template, element template.Element) error {
	...
	if !ok {
		return fmt.Errorf(
			"folio: stand-in data: element %s: element kind %q has no stand-in rule; refusing rather than silently dropping the data paths it may carry",
...
func (g *standInGenerator) collectTextValue(element template.Element) error {
	...
	if err != nil {
		return fmt.Errorf("folio: stand-in data: element %s value: %w", element.ID, err)
	}
...
func (g *standInGenerator) walk(node expr.Expr, demand standInDemand, site standInSite) error {
	...
	return fmt.Errorf(
		"folio: stand-in data: %s: expression node %T is unknown to the stand-in generator; refusing rather than silently dropping the data paths inside it",
		site.located(), node,
	)
}
...
func (g *standInGenerator) document() (map[string]any, error) {
	...
	for _, key := range slices.Sorted(maps.Keys(g.paths)) {
		...
		if !ok {
			...
			// than a panic (AD-14).
			return nil, fmt.Errorf("folio: stand-in data: path %q has no admitted stand-in value", key)
		}
		node := root
		for index, segment := range path.segments[:len(path.segments)-1] {
			...
			if !isObject {
				return nil, fmt.Errorf(
					"folio: stand-in data: path %q needs %q to be an object, but %q is itself a bound value; no stand-in document satisfies both",
					key, prefix, prefix,
				)
			}
			node = next
		...
		if _, exists := node[leaf]; exists {
			...
			// value instead.
			return nil, fmt.Errorf("folio: stand-in data: path %q is already occupied; no stand-in document satisfies both", key)
		}
