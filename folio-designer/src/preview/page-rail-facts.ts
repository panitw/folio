...
// are folio's answers to them, standing in for a pdf.js viewer application
// folio does not have and is not going to acquire.
...
// other components a thumbnail finished; folio has no such components, and the
...
// Read once, for a localised page label folio does not render (it supplies its
...
// FOLIO'S OWN ACCESSIBILITY AND PRESENTATION RULES, HANDED TO A DOM PDF.JS BUILT
// FOR A DIFFERENT APPLICATION.
//
// The vendored class builds `div.thumbnailImageContainer[role=button][tabindex]`
// because in pdf.js that element IS the control. Here the control is folio's own
...
// which writes `aria-current="page"` ON A SECOND ELEMENT inside folio's already
