...
describe('engine client protocol and lifecycle', () => {
  ...
  it('fails closed when every operation receives surplus table metadata', async () => {
    ...
    worker.ready()
			...
			worker.emit(operation === 'render' ? { ...base, bytes: new Uint8Array([9]).buffer, preview: { revision: 1, identity: 'a'.repeat(64), pdfSha256: 'b'.repeat(64), diagnostics: [] } } : operation === 'identity' ? { ...base, preview: { revision: 1, identity: 'a'.repeat(64) } } : operation === 'serialize' || operation === 'asset' || operation === 'stand-in-data' ? { ...base, bytes: new Uint8Array([9]).buffer } : operation === 'parameter-references' ? { ...base, parameterReferences: { revision: 1, names: [] } } : base)
