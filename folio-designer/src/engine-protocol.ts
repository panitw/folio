...
const isCanvas = (value: unknown): value is CanvasProjection => {
  ...
  let priorBand = -1
	...
	if (!isRecord(component) || !hasOnly(component, ['id', 'type', 'band', 'x', 'y', 'width', 'height', 'resizable', 'authored', 'value', 'binding', 'visibleIf', 'fontFamily', 'fontSize', 'lineSpacing', 'bold', 'italic', 'align', 'valign', 'color', 'background', 'borderWidth', 'borderColor', 'borderEdges', 'paddingTop', 'paddingRight', 'paddingBottom', 'paddingLeft', 'tableBind', 'columns', 'textPaint', 'image', 'imageUnavailable', 'barcode', 'barcodeUnavailable', 'qrcode', 'qrcodeUnavailable', 'belowSectionBreak']) || typeof component.id !== 'string' || component.id.length === 0 || component.id.length > MAX_ENGINE_ELEMENT_ID_LENGTH || ids.has(component.id) || !componentTypes.includes(component.type as string) || !bandNames.includes(component.band as string) || typeof component.resizable !== 'boolean' || !['x', 'y', 'width', 'height'].every((key) => typeof component[key] === 'number' && Number.isSafeInteger(component[key]) && (component[key] as number) >= 0)) return false
    ...
    if (component.belowSectionBreak === undefined ? value.sectionBreak !== undefined && component.band === 'content' : typeof component.belowSectionBreak !== 'boolean' || value.sectionBreak === undefined || component.band !== 'content') return false
