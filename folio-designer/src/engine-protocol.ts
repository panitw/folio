...
export function parseRequest(value: unknown): EngineRequest | undefined {
  ...
  if (needsPayload !== (value.payload !== undefined)) return undefined
...
export function parseInbound(value: unknown): EngineInbound | undefined {
  ...
  if (value.ok && hasOnly(value, ['protocolVersion', 'kind', 'requestId', 'ok', 'snapshot', 'bytes']) && isSnapshot(value.snapshot) && (value.bytes === undefined || isArrayBuffer(value.bytes) && value.bytes.byteLength <= MAX_ENGINE_PAYLOAD_BYTES)) return value as EngineSuccess
