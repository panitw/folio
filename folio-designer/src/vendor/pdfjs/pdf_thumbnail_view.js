...
// FOLIO MODIFICATION 1 of 3 — `pdfjs-lib` is an alias that only exists inside
// pdf.js's own build. Folio resolves the same two exports from the published
...
// FOLIO MODIFICATION 2 of 3 — the `AppOptions` import is deleted. It is read at
// exactly the two sites below, each supplying a default for a value that is
// already a constructor option, so dropping it removes app_options.js (1,082
// lines) from the closure without changing behaviour.

const DRAW_UPSCALE_FACTOR = 2; // See comment in `PDFThumbnailView.draw` below.
const MAX_NUM_SCALING_STEPS = 3;
const THUMBNAIL_WIDTH = 126; // px
// FOLIO MODIFICATION 3 of 3 (declaration) — app_options.js's own values for
// `maxCanvasPixels` (2 ** 25) and `maxCanvasDim` (32767) at tag v6.2.108.
const DEFAULT_MAX_CANVAS_PIXELS = 2 ** 25;
const DEFAULT_MAX_CANVAS_DIM = 32767;
...
class PDFThumbnailView extends RenderableView {
  ...
  constructor({
    ...
    // FOLIO MODIFICATION 3 of 3 — the two `AppOptions.get(...)` fallbacks are
    // replaced by the constants declared above, which carry pdf.js's own
    // upstream defaults for these options verbatim.
    this.maxCanvasPixels = maxCanvasPixels ?? DEFAULT_MAX_CANVAS_PIXELS;
    this.maxCanvasDim = maxCanvasDim || DEFAULT_MAX_CANVAS_DIM;
