...
## Why folio vendors source instead of importing it
...
|---|---|---|---|---|
| `pdf_thumbnail_view.js` | 556 → 569 | 17,105 → 17,986 | `8bb39945f9199f8c35fc1cb7999dc0542ab5859691365dab15e43be241093526` | `2e7941b046028133460ca997c18e01d62034b0b91c7cd2a4c606c45d3e6beb89` |
...
Beside each `.js` sits a hand-written `.d.ts` sibling. Those are folio's own files, not vendored bytes,
and they declare only the surface folio drives. They exist because `allowJs` is false and `strict` is
on, so an undeclared `.js` import is a hard `TS7016`.

## Every modification, and why

Each is marked in the source with a `FOLIO MODIFICATION n of 3` comment naming its reason, so the file
...
1. **`import … from "pdfjs-lib"` → `from 'pdfjs-dist/build/pdf.mjs'`** (upstream line 24).
   ...
   both confirmed exported from `pdfjs-dist/build/pdf.mjs`, the module folio's viewer already loads, so
...
2. **The `import { AppOptions } from "./app_options.js"` line is deleted** (upstream line 26).
   ...
   application configuration, none of which folio has — into the closure.
...
3. **The two `AppOptions.get(...)` fallbacks become declared constants** (upstream lines 95-96).
   ...
   This is immaterial to folio and must not be "fixed" by re-adding UA sniffing: `App.css`'s `.app-shell`
   ...
   whole behaviour. A future re-vendor that widens folio to a touch viewport must revisit this line.
...
`App.tsx`'s `previewViewState` is folio's single page-state authority, and a vendored file with its own
...
the top), and `app_options.js`, `ui_utils.js`, `event_utils.js` and `pdf_rendering_queue.js` — folio
...
4. Re-apply the three modifications above, each with its `FOLIO MODIFICATION` comment. If upstream has
...
6. Re-check the `.d.ts` siblings against the new source: the constructor options, the three collaborator
   call sites and the public methods folio drives.
