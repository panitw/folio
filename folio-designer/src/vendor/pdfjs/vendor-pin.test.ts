...
describe('the vendored PDF.js pin', () => {
  ...
  it('records every deviation from upstream that the source actually carries', () => {
    ...
    // the FOLIO MODIFICATION comments, which necessarily NAME what they removed.
    expect(source.match(/FOLIO MODIFICATION \d of 3/g) ?? []).toHaveLength(4)
    ...
    expect(fs.readFileSync(path.join(vendorDir, 'renderable_view.js'), 'utf8')).not.toContain('FOLIO MODIFICATION')
