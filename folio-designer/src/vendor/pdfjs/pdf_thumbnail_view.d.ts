...
// and `linkService.pagesCount` — so folio's adapters are three tiny objects
// rather than the pdf.js viewer application those names come from.
//
// `pdfPage` is `unknown` deliberately. The only page type folio has is the one
...
export declare class PDFThumbnailView extends RenderableView {
  ...
  readonly id: number
  // The three elements the class builds and appends to `container`. Folio reads
  // them only to hand its own presentation and accessibility rules to a DOM
  // pdf.js built for a viewer application folio does not have.
