...
function ComponentProperties({ components, onCommit, documentGeneration, propertyError, onEditTable }: { components: ReadonlyArray<PanelComponent>; onCommit: CommitProperties; documentGeneration: number; propertyError?: PropertyCommitError; onEditTable: (id: string) => void }) {
  ...
  if (components.length === 1 && types.has('text')) fields.push(['value', 'Text value'], ['expression', 'Text expression'])
...
function PropertyDraft({ label, field, components, ids, onCommit, documentGeneration, error }: { label: string; field: PropertyField; components: ReadonlyArray<PanelComponent>; ids: ReadonlyArray<string>; onCommit: CommitProperties; documentGeneration: number; error?: PropertyCommitError }) {
  ...
  return <div className="property-editor"><label>{label}<input aria-label={label} aria-description={same ? undefined : 'Mixed value'} aria-invalid={error ? 'true' : undefined} aria-errormessage={errorId} inputMode={label.includes('(pt)') ? 'decimal' : undefined} value={draft} placeholder={same ? undefined : 'Mixed'} disabled={pending} onChange={(event) => setDraft(event.target.value)} onBlur={() => void commit()} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); void commit() } if (event.key === 'Escape') { event.preventDefault(); revert(); (event.target as HTMLInputElement).blur() } }} /></label>{field !== 'x' && field !== 'y' && field !== 'width' && field !== 'height' && field !== 'value' && field !== 'expression' && <button type="button" className="property-action" disabled={pending} onMouseDown={(event) => event.preventDefault()} onClick={() => void submit({ field, operation: 'clear' }, true)}>Clear {label}</button>}{canNull && <button type="button" className="property-action" disabled={pending} onMouseDown={(event) => event.preventDefault()} onClick={() => void submit({ field, operation: 'null' }, true)}>Set {label} null</button>}{error && <p id={errorId} role="alert" className="property-error">{error.elementId ? `${error.elementId}: ` : ''}{error.dataPath ? `${error.dataPath}: ` : ''}{error.message}</p>}</div>
}
