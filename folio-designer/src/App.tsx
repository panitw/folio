...
export default function App({ engine, fileAccess, sampleFileAccess, imageFileAccess, initialSnapshot, initialSampleData, blankBytes, initializationError, offlineState = 'unavailable', loadState, payload, engineState = 'waiting', onRetry = () => undefined }: AppProps = {}) {
  ...
  return <div className={`app-shell${mode === 'preview' ? ' app-shell-preview' : ''}`} aria-label="Folio designer application shell" aria-busy={fileBusy}>
    ...
    <div className="workbench" id="future-features">
      ...
      </main> : <main className="preview-region" aria-label="Preview region">{!standInNotice && <div className="preview-heading"><p>{noDataPreview ? 'NO-DATA LAYOUT PREVIEW' : previewStatus === 'current' ? 'EXACT LOCAL PRODUCTION PDF' : 'LOCAL PDF PREVIEW'}</p>{/* STORY 13.5 — THE SECOND WAY BACK TO DESIGN IS GONE. The document
