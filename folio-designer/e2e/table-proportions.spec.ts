...
test('saved ratios, formulas and expanded footers keep aligned controls and exact canvas/browser/native PDF geometry', async ({ page }, testInfo) => {
  ...
  const binary = testInfo.outputPath('folio'); const file = testInfo.outputPath('proportions.folio'); const data = testInfo.outputPath('proportions.json'); const output = testInfo.outputPath('native.pdf')
  writeFileSync(file, saved); writeFileSync(data, sample)
  execFileSync('go', ['build', '-o', binary, './cmd/folio'], { cwd: goRoot, stdio: 'pipe' })
