...
type PdfSaveProbe = typeof window & {
  __folioPdfBytes?: number[]
  __folioPdfPicker?: { suggestedName: string; types: ReadonlyArray<{ description: string; accept: Record<string, string[]> }> }
  __folioTemplateWrites?: number
}
...
test('the activation-gated tier writes exactly the bytes the displayed producer digest covers and never reaches the held .folio handle', async ({ page }) => {
  await page.addInitScript(([rawTemplate, rawSample]: [number[], number[]]) => {
    ...
    const held = {
      ...
      createWritable: async () => ({ write: async () => { probe.__folioTemplateWrites = (probe.__folioTemplateWrites ?? 0) + 1 }, close: async () => undefined }),
    }
    const sample = {
      name: 'sample.json',
    ...
    const saved = {
      ...
      createWritable: async () => ({ write: async (written: ArrayBuffer) => { probe.__folioPdfBytes = Array.from(new Uint8Array(written)) }, close: async () => undefined }),
    }
    ...
    Object.assign(window, {
      ...
      showSaveFilePicker: async (options: PdfSaveProbe['__folioPdfPicker']) => { probe.__folioPdfPicker = options; return saved },
    })
  ...
  const picker = await page.evaluate(() => (window as PdfSaveProbe).__folioPdfPicker)
  ...
  const written = await page.evaluate(async () => {
    const bytes = Uint8Array.from((window as PdfSaveProbe).__folioPdfBytes ?? [])
