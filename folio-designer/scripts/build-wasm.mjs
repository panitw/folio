...
// A change to any page therefore renames ALL three, which is what keeps every
...
const canonicalDocumentation = documentationPages.map(([key, stem]) => {
  ...
  if (!existsSync(source)) throw new Error(`docs/${stem}.html is missing; Folio Designer bundles it as a precached documentation page`)
...
// All three URLs are exported so all three stay in Vite's asset graph even
// though the application links only the guide; the other two are reached from
