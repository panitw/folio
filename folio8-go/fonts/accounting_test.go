...
// (folio8-go/shipped_faces_test.go:389-444): a cardinality pre-check, a
...
// `go test ./...` on every commit. folio8-go/fontgen_matrix_test.go is
...
// manifest, so folio8-go/fontgen_matrix_test.go replays its instancing —
...
type noticeRecord struct {
	dir            string // e.g. "roboto", relative to folio8-go/fonts/
...
// folio8-designer/src/font-catalogue.test.ts:139-152 and
	...
	if len(found) != 1 {
		t.Fatalf(
			"%s: the %s row matched %d times, and this parser accepts exactly one.\n"+
				...
				"Either restore the row to the shape every other folio8-go/fonts/*/NOTICE.md uses, or re-derive "+
...
// directory: a future folio8-go/fonts/testdata/ — Go's conventional fixture
...
// readNoticeRecords walks every subdirectory of folio8-go/fonts/ that holds a
...
//
// ONE LEVEL, NOT A RECURSIVE WALK: folio8-go/fonts/ is flat by layout — one
...
func readNoticeRecords(t *testing.T) []noticeRecord {
	...
	if err != nil {
		t.Fatalf("could not read the folio8-go/fonts/ package directory: %v", err)
	}
	...
	for _, entry := range entries {
		...
		if err != nil {
			t.Fatalf("could not read folio8-go/fonts/%s/: %v", dir, err)
		}
		binaries := []string{}
		...
		if err != nil {
			t.Fatalf(
				"folio8-go/fonts/%s/ holds font binar(y/ies) %s but no readable NOTICE.md (%v).\n"+
		...
		if len(binaries) != 1 || binaries[0] != named {
			t.Fatalf(
				"folio8-go/fonts/%s/ holds font binar(y/ies) %s, but %s names only `%s` as its **Shipped file**. "+
					"Every font binary under folio8-go/fonts/ needs its own directory and its own NOTICE — a "+
	...
	if len(records) == 0 {
		t.Fatal(
			"the walk of folio8-go/fonts/ found NO face directory at all, so everything below it would be " +
	...
	if len(missing) > 0 {
		...
		t.Fatalf(
			"THE SILENT DIRECTION: %d of the %d face(s) fonts.Shipped() carries have NO provenance record "+
				...
				"Add folio8-go/fonts/<dir>/ holding the face, its LICENSE and a NOTICE.md in the shape the four "+
...
// BOUNDARY RATHER THAN CHOSEN. folio8-go/fontgen_matrix_test.go needs the same
...
//     manifest, so folio8-go/fontgen_matrix_test.go replays
