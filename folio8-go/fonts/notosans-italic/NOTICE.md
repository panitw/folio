# NotoSans-Italic.ttf — shipped production face (Latin, Story 11.1)
...
This is one of Story 11.1's seven new shipped production faces: folio8-go's bundled `FontSet`, keyed as
`"Noto Sans Italic"`.

**THIS FILE IS BYTE-IDENTICAL TO `folio8-designer/public/fonts/notosans-italic/NotoSans-Italic.ttf`
ON PURPOSE, AND THE BROWSER'S COPY DOES NOT FOLLOW THIS ONE AUTOMATICALLY.** This engine-side copy
is the one `tools/fontgen/instance_faces.py` writes and `make fonts-verify` re-derives and compares;
the browser's copy is a hand-made mirror that no generator writes and no derivation replays. Whoever
...
loudly. `folio8-designer/src/font-binary-identity.test.ts` digest-ties the two copies.
...
| item | value |
|---|---|
| **Shipped file** | `NotoSans-Italic.ttf` |
| **sha256 of the SHIPPED (produced) file** | `b3f9077e2c43979d1509bead91c46068742f16f3dd131b8005149451aaf40338` |
| Size | 663,520 bytes |
| Instance | Italic — `wght=400`, `wdth=100` (this face has TWO axes) |
| Declared PostScript name (`name[6]`) | `NotoSansItalic-Italic` |

## The exact invocation, verbatim

```sh
SOURCE_DATE_EPOCH=1451606400 fonttools varLib.instancer --update-name-table \
    NotoSans-Italic-VF.ttf wght=400 wdth=100 -o NotoSans-Italic.ttf
```
...
`folio8-go/fontgen_matrix_test.go` (`//go:build matrix`) proves the committed file still reproduces from
the upstream source.

## The PostScript name is upstream-determined, and it is NOT a typo

`name[6]` of this face reads `NotoSansItalic-Italic` — not `NotoSans-Italic`. fontTools composes the instanced
...
discipline exists to forbid. `folio8-go/shipped_faces_test.go`'s `shippedFaceSpecs` asserts `name[6]`
