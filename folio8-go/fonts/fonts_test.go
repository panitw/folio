...
type robotoCut struct {
	...
	Key string
	// Dir and File locate the cut, relative to folio8-go/fonts/ on this
	// side and folio8-designer/public/fonts/ on the other. The two trees
...
// robotoCuts is every Roboto face folio8-go ships. Story 16.8 shipped the
...
// e688a215…, 347.6 KiB) and a test fixture (`folio8-go/testdata/fonts/
...
func TestShippedRobotoMatchesDesignerCatalogue(t *testing.T) {
	...
	// folio8-go/testdata/fonts/Roboto-Regular.ttf IS A DIFFERENT CUT and
	...
	for _, cut := range robotoCuts {
		t.Run(cut.Key, func(t *testing.T) {
			...
			designerPath := filepath.Join("..", "..", "folio8-designer", "public", "fonts", cut.Dir, cut.File)
			...
			if shippedDigest != designerDigest {
				t.Fatalf(
					"folio8-go's embedded %q (sha256 %s, %d bytes) is NOT byte-identical to the designer's "+
			...
			if testFixtureDigest == shippedDigest {
				t.Fatalf(
					"folio8-go/testdata/fonts/Roboto-Regular.ttf (the Apache-2.0 licence-signature fixture) is "+
