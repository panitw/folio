...
func TableColumns(t *Template, tableID string) (TableColumnsProjection, error) {
	...
	if tableID == "" || len(tableID) > 128 {
		return TableColumnsProjection{}, fmt.Errorf("folio8: table id is invalid")
	}
	_, _, _, element, err := findComponent(t, tableID)
	if err != nil {
		return TableColumnsProjection{}, fmt.Errorf("folio8: table was not found")
	}
	if element.Type != template.ElementTable || !element.Table.Set || element.Table.Null {
		return TableColumnsProjection{}, fmt.Errorf("folio8: component is not a table")
	}
	if len(element.Table.Value.Columns) > maxTableColumns {
		return TableColumnsProjection{}, fmt.Errorf("folio8: table has too many columns for editor projection")
	}
	...
	if collection == "" || !rootCollectionPath.MatchString(collection) || len(collection) > 256 || alias == "" || len(alias) > 64 || !boundedIdentifier.MatchString(alias) || reservedRowAlias(alias) {
		return TableColumnsProjection{}, fmt.Errorf("folio8: table configuration cannot be projected")
	}
	...
	for i, column := range element.Table.Value.Columns {
		...
		if utf8.RuneCountInString(column.Label) > 256 || widths[i] <= 0 || len(column.ID) == 0 || len(column.ID) > 128 {
			return TableColumnsProjection{}, fmt.Errorf("folio8: table column cannot be projected")
		}
		align := "left"
