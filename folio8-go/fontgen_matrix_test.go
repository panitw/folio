...
//
// SEVEN of the ELEVEN faces under folio8-go/fonts/ are static instances
...
// folio8-go/fonts/accounting_test.go, which runs untagged on every commit.
...
//     ./...` and `go vet -tags=matrix ./...` in the folio8-go job — so it
...
func TestShippedFacesReproduceFromUpstream(t *testing.T) {
	...
	if _, err := os.Stat(sources); err != nil {
		t.Fatalf(
			"the upstream variable font sources are not at %s, so the shipped faces' derivation cannot be "+
				...
				"static-upstream route, accounted for by folio8-go/fonts/accounting_test.go.\n\n"+
	...
	// takes away, which is "the faces folio8-go ships reproduce" — and `3 of 3`
	...
	// TestEveryShippedFaceIsAccountedForByExactlyOneRoute in folio8-go/fonts
	...
	// seven while this pin grows by four, and folio8-go/fonts/accounting_test.go
	...
	if len(derived) != wantDerivedShippedFaces {
		t.Fatalf(
			"%d of the %d face(s) fonts.Shipped() carries are derived by tools/fontgen (%s); this test is "+
				...
				"deliberately, and check folio8-go/fonts/accounting_test.go agrees about the other route.",
...
//
// THE SAME READER EXISTS IN folio8-go/fonts/accounting_test.go, AND THE
