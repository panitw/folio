...
// deleting Bands.PageFooter.Elements from fontChainBands, the whole folio8-go
...
func TestComponentFailureBoundsMatchTheHostsOwnLiterals(t *testing.T) {
	path := filepath.Join(repoRootFromTest(t), "folio8-go", "wasm", "cmd", "engine", "main.go")
...
// whole folio8-go suite green. Every other embed test either dedupes first
...
// wd folio8-go, `go test -run 'VariableFace|Fvar|EmbedFontFamily' ./...`):
...
// 2026-09-03, wd folio8-go, `go test -run 'ContradictedLicence|StaticFace' ./...`):
