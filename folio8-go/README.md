...
module root) and `package fonts` (the shipped face data, `folio8-go/fonts/`)
...
if your own code imports `folio8-go/fonts` and calls `Shipped()`.
...
straight to the `io.Writer` you give it, and folio8-go's own code never opens
a temporary file to get there — that's what makes serving a PDF from an HTTP
handler with no scratch file possible. What folio8-go does not do is *forbid*
...
- **A top-level `"params"` key in your report data is legal, ordinary JSON,
  and simply unreachable by any binding.** It is your data; folio8-go does
...
folio8-go's whole reason to exist is that the same template and the same
inputs produce the same bytes, every time. `folio8-go/go.mod` pins an exact
Go toolchain (`toolchain go1.26.0`) for that reason.

**Where the golden hashes actually live.** folio8-go's own regression fixtures
(`folio8-go/testdata/`) and the cross-platform byte-identity matrix
(`hashmatrix/`, verified at each epic boundary — see
[What's not here yet](#whats-not-here-yet)) are what pin folio8-go's output
...
**That pin only binds folio8-go's OWN build.** Go's `toolchain` directive
binds the *main* module of whatever `go build` you run — not every module in
the dependency graph. If you `go get` folio8-go into your own application,
your application is the main module, and **your own `go.mod`'s `toolchain`
line (or your `GOTOOLCHAIN` setting) decides which Go compiles folio8-go**,
not folio8-go's own pin. An integrator who records folio8-go's golden hashes
in their own test suite, expecting them to hold indefinitely, needs to pin
their *own* toolchain the same way — folio8-go's pin alone does not do it for
them.

(If you look at `folio8-go/go.mod`, you'll notice the `go` directive sits
