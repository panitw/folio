...
// directory holding both folio8-go/ and lint/ — the same pattern
...
func TestCorpusRegeneratedMatchesCommitted(t *testing.T) {
	...
	if string(regeneratedJSON) != string(committed) {
		...
		if len(committedItems) != len(regenerated) {
			t.Fatalf("fixtures/thai-break-corpus/corpus.json is stale: committed has %d items, buildItems() produces %d — run `cd folio8-go && go run ./cmd/gencorpus && go run ./cmd/genbreaks` from the module root and commit the result", len(committedItems), len(regenerated))
		}
		t.Fatalf("fixtures/thai-break-corpus/corpus.json does not byte-match buildItems()'s output (same item count, %d, but different content) — run `cd folio8-go && go run ./cmd/gencorpus && go run ./cmd/genbreaks` from the module root and commit the result", len(regenerated))
	}
}
