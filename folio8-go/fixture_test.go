...
// folio8-go/ and fixtures/. Duplicated (rather than shared) with
...
func repoRootFromTest(t *testing.T) string {
	...
	for {
		...
		if parent == dir {
			t.Fatalf("could not find repo root (a directory containing both folio8-go/ and fixtures/) walking up from %s", dir)
		}
		dir = parent
	}
}
...
func TestMultiScriptFallbackGoldenFixture(t *testing.T) {
	...
	if string(inputFolio8Bytes) != multiScriptTestTemplateJSON {
		t.Fatalf(
			"%s has drifted from folio8-go/multiScriptTestTemplateJSON (render_test.go) — the two are "+
...
func TestRenderMatchesComponentAssetImportGoldenFixture(t *testing.T) {
	...
	if string(inputFolio8Bytes) != componentAssetImportTemplateJSON {
		t.Fatalf(
			"%s has drifted from folio8-go/componentAssetImportTemplateJSON (render_test.go) — "+
