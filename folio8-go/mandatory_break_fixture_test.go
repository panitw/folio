...
func TestMandatoryBreakSemanticAcceptance(t *testing.T) {
	...
	//     folio8-go would stop reporting, which is the DW-23 shape (one
...
func TestMandatoryBreakGoldenFixture(t *testing.T) {
	...
	if string(inputBytes) != mandatoryBreakTemplateJSON {
		t.Fatalf(
			"%s has drifted from folio8-go/mandatoryBreakTemplateJSON (mandatory_break_template.go) — the two are "+
