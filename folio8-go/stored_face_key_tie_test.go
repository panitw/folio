...
// `folio8-designer/src/font-store.test.ts`, in the describe block
...
// written in `folio8-designer/src/font-store.test.ts` as `goDerivedAssetKey`.
...
// `folio8-designer/src/test/sfnt-fixture.ts` builds for
...
// designer's half is `folio8-designer/src/font-store.test.ts`.
...
func TestEmbedFontFamilyKeysByTheSameDerivationTheTiePins(t *testing.T) {
	...
	if _, ok := tpl.doc.Assets[want]; !ok {
		...
		t.Fatalf("embedFontFamily stored the face under %v, not under the SHA-256 of its bytes (%s).\n"+
			"The designer's machine store keys by that digest — see `folio8-designer/src/font-store.ts` and the tie above — so a document keyed any other way makes every store hit a hit on bytes the document does not hold.",
			keys, want)
	}
}
