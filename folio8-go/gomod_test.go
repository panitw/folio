...
//	story {github.com/panitw/folio8/folio8-go, github.com/boxesandglue/
...
var wantModuleGraph = []string{
	"github.com/panitw/folio8/folio8-go",
...
// (folio8-go/matrix_test.go, //go:build matrix) still builds every target
// from the host, where `go list -m all` is available.
func goListModuleNames(t *testing.T) []string {
	t.Helper()
	if runtime.GOOS == "js" {
		t.Skip("js/wasm has no os/exec (\"go\" is not on $PATH); covered by the cross-target matrix harness (folio8-go/matrix_test.go)")
	}
...
func TestDiffModuleSetsCatchesExtraAndMissing(t *testing.T) {
	want := []string{"github.com/panitw/folio8/folio8-go", "github.com/boxesandglue/textshape"}

	extra, missing := diffModuleSets([]string{"github.com/panitw/folio8/folio8-go"}, want)
