...
var goldenDigestSearchScope = []string{"fixtures", filepath.Join("folio8-go", "byte_neutrality_test.go")}
...
func TestGoldenDigestAgreesAtEveryDeclaredSite(t *testing.T) {
	...
	for _, fx := range goldenDigestRecord {
		...
		for _, site := range fx.sites {
			switch site.kind {
			case "second-literal":
				declared[filepath.ToSlash(filepath.Join("folio8-go", "byte_neutrality_test.go"))] = true
			default:
...
// both folio8-go/ and lint/ — the same D-000.5/AD-21 pattern the other
...
func repoRootForByteNeutrality(t *testing.T) string {
	...
	for {
		folio8Go, e1 := os.Stat(filepath.Join(dir, "folio8-go"))
		...
		if parent == dir {
			t.Fatalf("could not find the repo root (a directory containing both folio8-go/ and lint/) walking up from %s", dir)
		}
		dir = parent
	}
}
