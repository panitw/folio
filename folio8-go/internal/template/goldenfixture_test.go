...
func TestWorkedExampleMatchesGoldenFixture(t *testing.T) {
	...
	golden := mustReadFile(t, filepath.Join(root, "folio8-go", "testdata", "template", "golden", "worked-example.json"))
...
func TestAssetExampleMatchesGoldenFragment(t *testing.T) {
	root := repoRootFromTest(t)
	docBytes := mustReadFile(t, filepath.Join(root, "_bmad-output", "specs", "spec-folio", "folio-format.md"))
	golden := mustReadFile(t, filepath.Join(root, "folio8-go", "testdata", "template", "golden", "asset-example.json"))
...
func TestAssetExampleAndWorkedExampleExtractorsAreIndependent(t *testing.T) {
	...
	golden := string(mustReadFile(t, filepath.Join(root, "folio8-go", "testdata", "template", "golden", "asset-example.json")))
	workedGolden := string(mustReadFile(t, filepath.Join(root, "folio8-go", "testdata", "template", "golden", "worked-example.json")))
