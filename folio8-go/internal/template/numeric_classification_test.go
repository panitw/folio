...
// anywhere in folio8-format.md (M-3 measured 11 distinct keys, zero
...
// given a numeric literal anywhere in folio8-format.md — fences and
...
// extractNumericDocKeys scans folio8-format.md's fenced code blocks (the
...
// added to folio8-format.md after this test was written escapes silently
...
// assertion, over the field names folio8-format.md ACTUALLY documents
...
func TestNumericFieldClassificationInventory(t *testing.T) {
	...
	if tokensExtracted == 0 {
		t.Fatal("coverage witness: zero numeric key/value tokens extracted from folio8-format.md")
	}
	...
	if len(documented) < 12 {
		t.Fatalf("folio8-format.md documents at least 12 distinct numeric keys (M-3's 11, plus Story 7.2's lineSpacing); extraction found only %d — extractNumericDocKeys may have regressed", len(documented))
	}
	...
	if len(unclassified) > 0 {
		t.Fatalf("fields folio8-format.md documents with a numeric literal but numericFieldRegistry does not classify: %v", unclassified)
	}
	for _, f := range names {
...
// does not know about (simulating folio8-format.md growing a new
