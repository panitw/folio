...
func TestDriftGoToDoc(t *testing.T) {
	...
	goKeys, tagsReflected, err := extractGoKeys(filepath.Join(root, "folio8-go", "internal", "template", goKeyRegistrySource))
	if err != nil {
...
func TestDriftCatchesARemovedDocKey(t *testing.T) {
	root := repoRootFromTest(t)
	goKeys, _, err := extractGoKeys(filepath.Join(root, "folio8-go", "internal", "template", goKeyRegistrySource))
	if err != nil {
		t.Fatalf("extract Go keys: %v", err)
	}
...
func TestDriftASTMatchesRuntimeEmission(t *testing.T) {
	root := repoRootFromTest(t)
	astKeys, tagsReflected, err := extractGoKeys(filepath.Join(root, "folio8-go", "internal", "template", goKeyRegistrySource))
	if err != nil {
		t.Fatalf("extract Go keys: %v", err)
	}
	if tagsReflected == 0 {
