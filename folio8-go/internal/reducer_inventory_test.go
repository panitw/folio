...
// realModuleImportPath is folio8-go's own module path (go.mod:1),
...
const realModuleImportPath = "github.com/panitw/folio8/folio8-go"

// TestDecimalReducerInventoryIsExactlySumAndAvg is AC23: the set of
// functions in the folio8-go module that reduce a sequence of Decimal to
...
func TestDecimalReducerInventoryIsExactlySumAndAvg(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("reducer inventory over the folio8-go module: %v", err)
	}
	...
	if stats.FilesParsed == 0 {
		t.Fatal("vacuity guard: scanner's own stats report 0 files parsed under the folio8-go module root")
	}
	if !stats.DeclFound {
...
func reducerInventoryFixtureRoot(t *testing.T, variant string) string {
	t.Helper()
	return filepath.Join(filepath.Dir(moduleRoot(t)), "folio8-go", "testdata", "arch", "reducer-inventory", variant)
}
