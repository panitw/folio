...
func TestSubsetRejectsGlyphIDsTheFaceDoesNotHave(t *testing.T) {
	...
	last := uint16(n - 1)
	beyond := uint16(n)
	t.Logf("SUBJECT: folio8-go/testdata/fonts/Roboto-Regular.ttf, NumGlyphs=%d (valid ids 0..%d)", n, last)
...
// (folio8-go/testdata/fonts/Roboto-Regular.ttf, the same 10 shaped ids
