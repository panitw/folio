...
func TestVendorBoundaryDocumentExistsAndIsCited(t *testing.T) {
	...
	if gerr != nil {
		t.Fatalf("read folio8-go/go.mod: %v", gerr)
	}
	if !strings.Contains(string(goMod), "github.com/boxesandglue/textshape "+textshapeVersion) {
		t.Fatalf("folio8-go/go.mod does not require github.com/boxesandglue/textshape %s — internal/fontset/%s is measured against a version the module no longer pins, so every row in it is unverified", textshapeVersion, vendorBoundaryDoc)
	}
