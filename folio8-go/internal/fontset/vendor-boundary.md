...
This document is a claim about one dependency at one version. `folio8-go/go.mod` pins that version and
...
- **Subject:** `folio8-go/testdata/fonts/Roboto-Regular.ttf` — unitsPerEm 2048, 1,294 glyphs,
...
|---|---|---|
| `folio8-go/testdata/fonts/Roboto-Regular.ttf` | 1,294 | **0 mismatches** |
| `folio8-go/fonts/notosans/NotoSans-Regular.ttf` | 4,515 | **0 mismatches** |
| `folio8-go/fonts/notosansthai/NotoSansThai-Regular.ttf` | 467 | **0 mismatches** |
| `folio8-go/fonts/notosanssc/NotoSansSC-Regular.ttf` | 31,036 | **0 mismatches** |
| **total** | **37,312** | **0** |

So the swap changed no width, no `/W` array, no content stream and no golden.

**The guard that could not see it.** `folio8-go/internal/arch_test.go` matches the *spelling* of a type
...
syntactic guard reported **zero** under `folio8-go` while **four** float-typed value expressions stood
...
`folio8-go/internal/fontset/vendorboundary_test.go`, so this document cannot drift away from the
