...
// moduleRoot resolves folio8-go/ from this package's own directory
// (folio8-go/internal/), the same way TestNoFloat64UnderModule does.
...
func TestPageModelNamesNoPDFConcept(t *testing.T) {
	...
	for _, pf := range files {
		...
		for _, imp := range pf.file.Imports {
			...
			if !strings.Contains(path, "/folio8-go/internal/") {
				continue
			}
...
func firstPartyImportGraph(t *testing.T, internalDir string) map[string]map[string]bool {
	...
	err := filepath.WalkDir(internalDir, func(path string, d os.DirEntry, err error) error {
		...
		for _, imp := range f.Imports {
			...
			const marker = "/folio8-go/internal/"
...
func TestInternalLayoutDoesNotReachInternalPDF(t *testing.T) {
	...
	if !ok {
		t.Fatal("presence precondition: no package directory \"layout\" found under folio8-go/internal/ — AD-5's arrow is unassertable (AC2 creates this package)")
	}
	if len(direct) == 0 {
