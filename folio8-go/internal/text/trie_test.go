...
func loadWordlist(t *testing.T) []string {
	...
	path := filepath.Join(root, "folio8-go", "internal", "text", "wordlist", "words_th.txt")
	f, err := os.Open(path)
	if err != nil {
...
func TestTrieRegeneratedMatchesCommitted(t *testing.T) {
	...
	if string(regenerated) != string(compiledTrie) {
		t.Fatalf("data/thai_words.trie is stale (got %d bytes regenerated, %d committed) — run `cd folio8-go && go run ./cmd/gentrie` and commit the result", len(regenerated), len(compiledTrie))
	}
}
