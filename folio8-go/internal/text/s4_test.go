...
// package's js/wasm leg (folio8-go/wasmleg_test.go), which is what makes
...
func TestAC10ComputedBreaksMatchS4Basis(t *testing.T) {
	...
	if len(fixture) != len(items) {
		t.Fatalf("V10: fixture has %d entries, corpus has %d items — the fixture is stale (run `cd folio8-go && go run ./cmd/genbreaks` and commit the result)", len(fixture), len(items))
	}
