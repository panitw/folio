...
// matrixBuildDir returns folio8-go/.matrix-build, creating it if needed.
...
func matrixBuildDir(t *testing.T, root string) string {
	t.Helper()
	dir := filepath.Join(root, "folio8-go", ".matrix-build")
...
// buildRenderTestBinary builds folio8-go's own package test binary for
...
func buildRenderTestBinary(t *testing.T, root string, target matrixTarget, buildDir string) string {
	...
	cmd.Dir = filepath.Join(root, "folio8-go")
...
// no dependency on folio8-go, so this is an entirely separate build.
