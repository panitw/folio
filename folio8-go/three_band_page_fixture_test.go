...
func TestThreeBandPageGoldenFixture(t *testing.T) {
	...
	if string(inputBytes) != threeBandPageTemplateJSON {
		t.Fatalf("%s has drifted from folio8-go/threeBandPageTemplateJSON (three_band_page_template.go) — the two are supposed to be byte-identical, kept in sync by hand (font-text's and wrapped-text's precedent)", inputPath)
	}
