...
Colours are `#RRGGBB`; a colour that is not reaches render as an error (`STYLE_COLOR_INVALID`). There
is no colour-by-data: conditional *visibility* is in scope, conditional *formatting* is not. A `{{ }}`
placeholder found inside any string-valued style field (`background`, `color`, `fontFamily`,
`border.color`, `border.edges`) — or in a table's own `altRowBackground` above, the format's one
colour field outside `style` — is a **load error** naming the element and the field — a component's
condition turns it on or off (`visibleIf`), it never changes how the component looks. This is not
general style-field validation: a hex colour or a font-family name is still accepted at load with no
format check at all — the rejection is narrowly "does this string contain a placeholder", nothing
broader. Closed-set fields such as `align` and `valign` are governed by their own sets instead.
