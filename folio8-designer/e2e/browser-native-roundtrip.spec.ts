...
const goRoot = path.join(root, 'folio8-go')
...
// `folio8-go/starter_template_test.go`, guarded by that file's parse of the real
...
test('fresh authored sessions close exactly through admitted Preview and native folio8', async ({ browser }, testInfo) => {
  ...
  const evidence = {
    ...
    handEditedNativeTemplate: 'folio8-go/testdata/template/golden/worked-example.json (native test only; never uploaded by this browser test)',
  }
