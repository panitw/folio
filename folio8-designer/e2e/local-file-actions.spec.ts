...
const fixture = readFileSync(path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../../fixtures/statement-1/input.folio8'))
...
test('the fallback opens a local file, saves worker output as a download, and remains local while offline', async ({ page, context }) => {
  ...
  await (await chooser).setFiles({ name: 'statement.folio8', mimeType: 'application/json', buffer: fixture })
  await expect(page.locator('.document-name')).toHaveText('statement.folio8')
  const download = page.waitForEvent('download')
  await page.keyboard.press(process.platform === 'darwin' ? 'Meta+S' : 'Control+S')
  expect((await download).suggestedFilename()).toBe('statement.folio8')
  await expect(page.getByText(/Downloaded local file statement\.folio8/)).toBeVisible()
})
...
test('the activation-gated tier opens, Save As picks before the worker write, and writes opaque worker bytes', async ({ page }) => {
  await page.addInitScript((rawBytes) => {
    ...
    const handle = {
      name: 'native.folio8',
      getFile: async () => new File([bytes], 'native.folio8', { type: 'application/json' }),
  ...
  await expect(page.getByTestId('engine-snapshot')).toHaveText(/GO SNAPSHOT · REVISION 1/)
  await page.getByRole('button', { name: 'Open local template' }).click()
  await expect(page.locator('.document-name')).toHaveText('native.folio8')
  ...
  await expect(page.getByText('Saved locally as native.folio8')).toBeVisible()
})
...
test('a native Save As cancellation keeps the opened local identity and unsaved indicator honest', async ({ page }) => {
  await page.addInitScript((rawBytes) => {
    ...
    Object.assign(window, {
      showOpenFilePicker: async () => [{ name: 'cancelled.folio8', getFile: async () => new File([bytes], 'cancelled.folio8', { type: 'application/json' }), createWritable: async () => { throw new Error('not reached') } }],
  ...
  await expect(page.getByTestId('engine-snapshot')).toHaveText(/GO SNAPSHOT · REVISION 1/)
  await page.getByRole('button', { name: 'Open local template' }).click()
  await expect(page.locator('.document-name')).toHaveText('cancelled.folio8')
  await page.getByRole('button', { name: 'Save As' }).click()
  await expect(page.locator('.document-name')).toHaveText('cancelled.folio8')
