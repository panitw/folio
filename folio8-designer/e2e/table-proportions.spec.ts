...
test('exact extreme proportions remain editable through keyboard navigation and save/reopen', async ({ page }) => {
  ...
  for (const value of ['9007199254740.991', '9223372036854775.807']) {
    ...
    await (await choosing).setFiles({ name: 'exact-proportion.folio8', mimeType: 'application/json', buffer: saved })
    await expect(page.locator('.document-name')).toHaveText('exact-proportion.folio8')
...
test('saved ratios, formulas and expanded footers keep aligned controls and exact canvas/browser/native PDF geometry', async ({ page }, testInfo) => {
  ...
  const choosing = page.waitForEvent('filechooser'); await page.getByRole('button', { name: 'Open local template' }).click()
  await (await choosing).setFiles({ name: 'proportions.folio8', mimeType: 'application/json', buffer: saved })
  await expect(page.locator('.document-name')).toHaveText('proportions.folio8')
  ...
  const goRoot = path.resolve(import.meta.dirname, '../../folio8-go')
  const binary = testInfo.outputPath('folio8'); const file = testInfo.outputPath('proportions.folio8'); const data = testInfo.outputPath('proportions.json'); const output = testInfo.outputPath('native.pdf')
