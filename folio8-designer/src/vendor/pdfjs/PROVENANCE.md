...
folio8's offline release budget (`src/release-payload.ts`) caps the payload at `maximumCacheAssets = 65` (64 when this measurement was taken).
...
| licence | Apache License 2.0, unchanged. Full text at `folio8-designer/third-party-notices/pdfjs-dist/LICENSE-APACHE-2.0`. |

The tag and the `pdfjs-dist` version in `folio8-designer/package.json` are **the same number on purpose**:
...
1. Pick the tag. It must equal the `pdfjs-dist` version in `folio8-designer/package.json`; bump the
...
8. Update `folio8-designer/third-party-notices/pdfjs-dist/NOTICE` if the file list changed.
