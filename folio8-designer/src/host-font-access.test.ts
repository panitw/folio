...
describe('no code path in the designer reaches for host-installed fonts', () => {
  ...
  it('scans its own file, so the guard is not exempt from the rule it enforces', () => {
    expect(scannedPopulation(repoRoot), 'the host-font guard must be inside the population it scans').toContain('folio8-designer/src/host-font-access.test.ts')
    // AND SO IS THE SCANNER IT DRIVES, for the same reason.
    expect(scannedPopulation(repoRoot)).toContain('folio8-designer/scripts/host-font-access.mjs')
  })
  ...
  it('fails, naming the file and the line, on a source file that calls the API', () => {
    ...
    try {
      ...
      expect(result.findings.map((finding) => `${finding.file}:${finding.line}:${finding.api}`)).toEqual(['folio8-designer/src/offender.ts:2:query' + 'LocalFonts'])
      // The thrown message NAMES the file and the line, not merely a count.
      expect(() => assertNoHostFontAccess(scratch, { floor: 1 })).toThrow(/folio8-designer\/src\/offender\.ts:2/)
  ...
  it('reads an UNTRACKED file and reds on a Local Font Access call in one', () => {
    ...
    try {
      ...
      expect(population, 'the untracked file must be IN the population — that is the widening').toContain('folio8-designer/src/never-staged.ts')
      expect(population).toContain('folio8-designer/src/committed-clean.ts')

      // (b) AND THE SCAN REDS, NAMING path:line.
      expect(() => assertNoHostFontAccess(scratch, { floor: 1 })).toThrow(/folio8-designer\/src\/never-staged\.ts:2/)
