...
describe('the content address the store keys by', () => {
  ...
  //
  // The Go half is `folio8-go/stored_face_key_tie_test.go` — `TestStoredFaceKeyTie`,
  ...
  it('computes the same key Go derives for the same bytes', async () => {
    // THE BYTE-LENGTH GUARD, which `folio8-go/stored_face_key_tie_test.go`
    ...
    // bytes.
    expect(face.byteLength, 'the fixture moved; the Go-derived digest below must be re-derived on BOTH sides (see folio8-go/stored_face_key_tie_test.go), not adjusted').toBe(110)
