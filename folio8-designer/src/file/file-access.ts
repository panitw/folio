...
// It deliberately has no knowledge of the .folio8 document structure.
...
// with. Before Story 13.1 all three were `.folio8` literals in three different
...
// changing two of them and shipping a PDF named `.folio8`, or a `.pdf` written
...
export const folio8FileFormat: LocalFileFormat = { description: 'folio8 template', mimeType: 'application/json', extension: '.folio8' }
...
const knownFileFormats: ReadonlyArray<LocalFileFormat> = [folio8FileFormat, pdfFileFormat]
...
// requested one is appended, in both directions: `statement.folio8` is offered
// as `statement.pdf`, and `statement.pdf` is offered as `statement.folio8` —
// not `statement.pdf.folio8`. The asymmetric variant (strip only on the PDF
// path, because that is the direction this story needed) was considered and
// REJECTED: it produces `statement.pdf.folio8` from a title a PDF save just
...
// the format list removed BOTH — `report.folio8.pdf` lost `.pdf`, and the
// result then ended in `.folio8` and lost that too, so a `.folio8` save produced
// `report.folio8.folio8`. One match, one slice.
//
...
// `REPORT.FOLIO8` and `REPORT.folio8`.
//
...
// case-insensitive filesystem `REPORT.folio8` silently overwrites
// `REPORT.FOLIO8`, and on a case-sensitive one the author now holds two
...
export function localFileName(name: string, format: LocalFileFormat): string {
  ...
  // The untitled fallback runs AFTER the strip, not before: `.folio8` is a
  ...
  // `.folio8` saved as a `.folio8` returns unchanged above, which is the shipped
