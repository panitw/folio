...
describe('no forbidden font host reaches the scanned source', () => {
  ...
  it('fails, naming the file and the line, on a source file that contains a forbidden host', () => {
    ...
    try {
      ...
      git('init', '-q')
      const tree = path.join(scratch, 'folio8-designer', 'src')
      ...
      expect(result.findings).toEqual([{ file: 'folio8-designer/src/offender.ts', host: hosts[0], half: 'forbidden', line: 2, text: `const url = 'https://${hosts[0]}/css2?family=Inter'` }])
      // The thrown message NAMES the file and the line (AC4), not merely a count.
      expect(() => assertNoForbiddenFontHosts(scratch, { floor: 1 })).toThrow(/folio8-designer\/src\/offender\.ts:2/)
      // And the clean file is not reported, so the matcher discriminates.
      expect(result.findings.map((finding) => finding.file)).not.toContain('folio8-designer/src/clean.ts')
    } finally {
  ...
  it('refuses to report a clean population over an implausibly small one', () => {
    ...
    try {
      const git = scratchGit(scratch)
      git('init', '-q')
      const tree = path.join(scratch, 'folio8-go')
      fs.mkdirSync(tree, { recursive: true })
  ...
  it('fails, naming the file, the line and the half, on an allowed host used outside its declaring module', () => {
    ...
    try {
      ...
      const result = scanForbiddenFontHosts(scratch, { floor: 1 })
      expect(result.findings.map((finding) => finding.file)).toEqual(['folio8-designer/src/second-fetch-site.ts'])
      expect(result.findings[0].half).toBe('declared-only')
      expect(() => assertNoForbiddenFontHosts(scratch, { floor: 1 })).toThrow(/folio8-designer\/src\/second-fetch-site\.ts:1/)
  ...
  it('reads an UNTRACKED file and reds on a forbidden host in one, which is the property the widening buys', () => {
    ...
    try {
      ...
      expect(population, 'the untracked file must be IN the population — that is the widening, and everything below is downstream of it').toContain('folio8-designer/src/never-staged.ts')
      expect(population).toContain('folio8-designer/src/committed-clean.ts')

      // (b) AND THE SCAN REDS, NAMING path:line rather than a count.
      expect(() => assertNoForbiddenFontHosts(scratch, { floor: 1 })).toThrow(/folio8-designer\/src\/never-staged\.ts:2/)
      ...
      expect(result.files).toBe(result.tracked + result.untracked)
      expect(result.findings.map((finding) => `${finding.file}:${finding.line}`)).toEqual(['folio8-designer/src/never-staged.ts:2'])
    } finally {
      fs.rmSync(scratch, { recursive: true, force: true })
    }
  })
  ...
  it('refuses, naming the untracked half, when ONLY the --others listing cannot be obtained', () => {
    ...
    try {
      ...
      const population = scannedPopulation(scratch)
      expect(population).toContain('folio8-designer/src/committed-clean.ts')
      expect(population, 'the untracked half is really being read before it is broken').toContain('folio8-designer/src/never-staged.ts')
      ...
      // a broken git.
      expect(scannedPopulation(scratch)).toContain('folio8-designer/src/never-staged.ts')
    } finally {
      fs.rmSync(scratch, { recursive: true, force: true })
    }
  })
  ...
  it('refuses a repository whose index is empty, rather than reporting the untracked half as the tree', () => {
    ...
    try {
      ...
      git('add', 'folio8-designer/src/a.ts')
      expect(scannedPopulation(scratch)).toContain('folio8-designer/src/a.ts')
    } finally {
      fs.rmSync(scratch, { recursive: true, force: true })
    }
  })
  ...
  it('refuses an untracked nested repository it cannot recurse into, naming the path', () => {
    ...
    try {
      ...
      expect(scannedPopulation(scratch)).toContain('folio8-designer/src/committed-clean.ts')
      ...
      expect(() => scannedPopulation(scratch)).toThrow(/could not look/)
      expect(() => scannedPopulation(scratch)).toThrow(/folio8-designer\/src\/vendor\//)
  ...
  it('reads untracked files that are not ignored, and leaves ignored ones out', () => {
    ...
    try {
      ...
      fs.writeFileSync(path.join(scratch, '.gitignore'), 'folio8-designer/src/generated/emitted.ts\n')
      ...
      const population = scannedPopulation(scratch)
      expect(population, 'an untracked file nobody ignored is read').toContain('folio8-designer/src/authored.ts')
      expect(population, 'an untracked file the ignore list names is NOT read').not.toContain('folio8-designer/src/generated/emitted.ts')
      const result = scanForbiddenFontHosts(scratch, { floor: 1 })
  ...
  // `folio8-designer/src/generated/`, and those artifacts are kept out of git
  ...
  it('measures, rather than assumes, that the generated artifacts stay out of the widened population', () => {
    const generated = 'folio8-designer/src/generated'
  ...
  it.skipIf(!fs.existsSync(path.join(repoRoot, 'folio8-designer/src/generated', 'font-index.ts')))(
    ...
    () => {
      const fontIndex = path.join(repoRoot, 'folio8-designer/src/generated', 'font-index.ts')
