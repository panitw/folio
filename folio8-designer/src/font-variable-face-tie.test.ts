...
//   `folio8-go/testfont_embed_test.go:22,34` embeds
...
//   `folio8-go/component_commands_test.go:2139`
...
// THE SAME PATHS GO EMBEDS. Resolved through `../../folio8-go/...`, the idiom
// `src/engine-bounds-mirror.test.ts:44` established for a designer test reading
// the Go tree; thirteen designer test files already reference `folio8-go`.
const goTestdata = path.resolve(here, '..', '..', 'folio8-go', 'testdata', 'fonts')
...
const readFixture = (file: string): Buffer => {
  expect(fs.existsSync(file), `the shared fixture ${file} is not there; Go embeds it by this exact path (folio8-go/testfont_embed_test.go), so a tie that cannot read it is broken, not skipped`).toBe(true)
