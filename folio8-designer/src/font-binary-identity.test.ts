...
//      is declared from bytes IDENTICAL to the bytes `folio8-go/fonts/fonts.go`
...
const engineFontsDir = path.join(designerRoot, '..', 'folio8-go', 'fonts')
...
// D-8.5.2) from `folio8-designer/public/fonts` to THE WHOLE TRACKED REPOSITORY.
//
...
// committed under `folio8-go/`, under `lint/testdata/`, or anywhere else was
 ...
 *
 * Vitest runs under jsdom with cwd `folio8-designer/`, so `-C <root>` is
...
describe('the family names the browser is given are the files the engine measured with', () => {
  ...
  //
  // The second population was `folio8-designer/public/fonts` until Story 8.4h
  ...
  it('lets the asset licence gate see every font that reaches the runtime bundle', () => {
    ...
    expect([...reachedDirectories].sort(), 'the widened population must span the repository the licence gate walks, not one subtree')
      .toEqual(expect.arrayContaining(['folio8-designer', 'folio8-go', 'lint']))
  ...
  // WALKS, where before it was reported only from `folio8-designer/public/fonts`.
  ...
  it('reports a font the licence gate cannot see from anywhere it walks, not just public/fonts', () => {
    ...
    try {
      ...
      write('folio8-go/testdata/fonts/Sneaked.woff2', woff2)
      write('lint/testdata/licence/Sneaked.woff2', woff2)
      write('folio8-designer/src/assets/Sneaked.woff2', woff2)
      ...
      write('folio8-go/fonts/Legitimate.ttf', woff2)
      write('folio8-go/NOTICE.md', 'not a font\n')
      write('folio8-go/LICENSE', 'not a font\n')
      // And one inside `*/testdata/lint`, which the licence gate's own walk
      // SKIPS — so this guard must skip it too, or the two disagree.
      write('folio8-go/testdata/lint/embed-font/Skipped.woff2', woff2)
      // An UNTRACKED font-magic file: the gate excludes what git does not
      // track, and so must this. Written but never added.
      write('folio8-go/untracked-scratch/Ignored.woff2', woff2)

      execFileSync('git', ['-C', scratchRepo, 'add',
        'folio8-go/testdata/fonts/Sneaked.woff2',
        'lint/testdata/licence/Sneaked.woff2',
        'folio8-designer/src/assets/Sneaked.woff2',
        'folio8-go/fonts/Legitimate.ttf',
        'folio8-go/NOTICE.md',
        'folio8-go/LICENSE',
        'folio8-go/testdata/lint/embed-font/Skipped.woff2',
      ])
      ...
      expect([...committedFontsTheLicenceGateCannotSee(tracked, recognised, scratchRepo)].sort(), 'the widened guard must report a font-magic file with an unrecognised extension from ANY tracked directory the licence gate walks').toEqual([
        "folio8-designer/src/assets/Sneaked.woff2 (extension '.woff2')",
        "folio8-go/testdata/fonts/Sneaked.woff2 (extension '.woff2')",
        "lint/testdata/licence/Sneaked.woff2 (extension '.woff2')",
      ])

      // AND THIS IS WHAT THE WIDENING BOUGHT. The pre-8.4h population was
      // `folio8-designer/public/fonts`, which does not exist in this repository
      ...
      const narrowPopulation = path.join(scratchRepo, 'folio8-designer', 'public', 'fonts')
  ...
  it('declares each engine face from bytes identical to the ones fonts.Shipped() embeds', () => {
    ...
    for (const face of Object.keys(shippedHandWritten)) {
      ...
      expect(
        digest(browserFile),
        `the browser declares '${face}' from ${declaredPaths[face]}, which must be byte-identical to the file folio8-go embeds under that face name (${shippedHandWritten[face]}) — AD-17 makes the browser a rasterizer only, so a same-named different face fails silently`,
  ...
  // (this file, byte-identical; folio8-go/testdata/fonts/Roboto-Regular.ttf,
  ...
  // folio8-go/fonts/fonts_test.go's TestShippedRobotoMatchesDesignerCatalogue;
  ...
  it('ships the SAME Roboto bytes the catalogue declares — there is exactly one Roboto', () => {
    ...
    expect(
      ...
      `the designer's catalogue Roboto (${catalogueEngineRobotoFile}) must be byte-identical to the file folio8-go embeds as "Roboto" (${shipped.Roboto}) — a second cut under the same name would make "fontFamily": "Roboto" render differently depending on which path produced the document`,
    ).toBe(digest(engineFile))
  })
})
...
describe('the bytes behind a chrome family name are the face that name claims', () => {
  ...
  // half was already digest-pinned (against `folio8-go/fonts`, above); the
