...
// while having become false — the moment folio8-go shipped a different set.
const enginePath = path.join(here, '..', '..', 'folio8-go', 'fonts', 'fonts.go')
...
// folio8-go embeds as its fourth shipped face.
...
describe('the canvas paints with the faces the engine measured', () => {
  ...
  // which would have kept passing, while having become false, if folio8-go ever
  ...
  it('declares every engine face name, and asks the canvas for the ones the degrade stack carries', () => {
    ...
    // SAME bytes folio8-go embeds — are asserted on their own, immediately
  ...
  it('Roboto: the catalogue declares it, and its bytes are the ones fonts.Shipped() embeds', () => {
    ...
    // the browser-side twin of folio8-go/fonts/fonts_test.go's
    ...
    const engineFile = path.join(here, '..', '..', 'folio8-go', 'fonts', shippedPaths.Roboto ?? '')
    ...
    expect(
      ...
      'the designer catalogue\'s Roboto and the file folio8-go embeds as "Roboto" must be byte-identical — a second cut under the same name would make a document naming Roboto render differently depending on which path produced it',
