...
// `folio8-go/page_setup.go` declares, and the band-containment predicate
// `folio8-go/component_commands.go` declares. There is no shared source and no
...
// `folio8-go/internal/template/linespacing.go` and whose maximum that file
...
const goSources = {
  pageSetup: path.resolve(sourceDir, '../../folio8-go/page_setup.go'),
  lineSpacing: path.resolve(sourceDir, '../../folio8-go/internal/template/linespacing.go'),
  ...
  componentCommands: path.resolve(sourceDir, '../../folio8-go/component_commands.go'),
  ...
  locale: path.resolve(sourceDir, '../../folio8-go/internal/template/locale.go'),
  ...
  templateModel: path.resolve(sourceDir, '../../folio8-go/internal/template/model.go'),
  ...
  // inside `folio8-go`, and still the same module as every source above it —
