...
// folio8-go/component_commands.go: `a font chain named %q already exists`,
...
describe('the AD-17 corpus scan can see a NEW violation (Story 17.6)', () => {
  ...
  it('carries no quarantined designer job in ci.yml, and leaves folio8-go-known-red alone', () => {
    // POSITIVE CONTROLS FIRST, so the two absences below are real silence and
    // not a failed read of the wrong file.
    expect(workflow).toMatch(/^ {2}folio8-designer:$/m)
    expect(workflow).toMatch(/^ {2}folio8-go-known-red:$/m)
    expect(workflow).toMatch(/KNOWN_RED_TEST: "\^TestCorpusMeetsP6ExerciseFloors\$"/)
    expect(workflow).not.toMatch(/folio8-designer-known-red/)
