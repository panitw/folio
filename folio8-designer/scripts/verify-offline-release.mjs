...
export function verifyOfflineRelease(outputDir = dist, { wasmWitness = false, reportApproach = false } = {}) {
  ...
  if (!dictionaryRow || dictionaryRow.delivery !== 'embedded-in-engine' || dictionaryRow.assetUrl !== release.thaiDictionary.wasmUrl || dictionaryRow.bytes !== readFileSync(join(root, '..', 'folio8-go', 'internal', 'text', 'data', 'thai_words.trie')).byteLength || dictionaryRow.sha256 !== release.thaiDictionary.sha256) fail('S1 Thai dictionary row is not a real embedded witness')
  ...
  const thaiSource = readFileSync(join(root, '..', 'folio8-go', 'internal', 'text', 'data', 'thai_words.trie'))
...
function assertEngineWasmIsTreeIndependent() {
  ...
  try {
    ...
    const strayPath = `folio8-go/${basename(stray)}`
    let porcelain
