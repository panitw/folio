...
var servedModules = []servedModule{
	{dirName: "folio8-go", name: "folio8-go", shippedBy: "shipped"},
...
func Generate(repoRoot string) ([]Row, error) {
	...
	designerPackages, err := licence.ResolveNPMGraph(filepath.Join(repoRoot, "folio8-designer"))
	if err != nil {
		return nil, fmt.Errorf("resolve folio8-designer lockfile graph: %w", err)
	}
	for _, p := range designerPackages {
		...
		rows = append(rows, Row{Module: p.Path, Version: p.Version, Licence: p.Licence, Serves: "folio8-designer", ShippedBy: shippedBy})
	}
...
func assetServesLabel(relDir string) string {
	switch {
	case relDir == "folio8-go/testdata/fonts" || strings.HasPrefix(relDir, "folio8-go/testdata/fonts/"):
		return "folio8-go test fixture"
	case relDir == "folio8-go/fonts" || strings.HasPrefix(relDir, "folio8-go/fonts/"):
		return "folio8-go shipped"
	default:
...
// proved by construction: a font at folio8-go/assets/Stray.ttf, or at
// folio8-go/testdata/fonts/extra/Second.ttf, both stayed green under the
...
// evading-shape fixtures under folio8-go/testdata/lint/embed-font/,
...
const wordlistAssetDir = "folio8-go/internal/text/wordlist"
...
func resolveWordlistAssetRow(repoRoot string) (AssetRow, bool, error) {
	...
	return AssetRow{
		...
		Licence:   licenceLabel,
		Copyright: copyrightLine,
		Serves:    "folio8-go shipped (embedded dictionary)",
	}, true, nil
}
...
func Render(rows []Row) string {
	...
	b.WriteString("`folio8-designer/package-lock.json` graph appears below with its resolved\n")
