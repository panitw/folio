...
func collectLicenceSignals(text string) licenceSignals {
	...
	// dedup through the same `seen` map every other signal uses.
	//
	...
	for _, m := range spdxLineRE.FindAllStringSubmatch(text, -1) {
		...
		case family == FamilyCopyleft:
			...
			markExpressionFieldsSeen(seen, fields)
		default:
...
// markExpressionFieldsSeen marks EVERY whitespace-separated field of a
// RESOLVED SPDX expression as already-seen, so a later NAME or CLAUSE
// signal for one of the expression's own terms does not count a second
// time (Story 8.4j).
//
...
//
// NO GRAMMAR KNOWLEDGE IS NEEDED OR WANTED. Operators are marked too and
// are inert, because no entry of licenceNames or licenceClauses has the
// id "OR" or "AND". Marking the fields rather than the parsed terms
// keeps this function free of any second opinion about SPDX syntax.
//
...
// header over a GPL body still refuses as copyleft (measured).
func markExpressionFieldsSeen(seen map[string]bool, fields []string) {
