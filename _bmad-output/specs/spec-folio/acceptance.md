...
| R4 | Compressor output is stable by observation, not by contract — a future Go release could invalidate every recorded golden hash downstream. Closed by invoking no compressor at all (`no-compressor-import`, D-1.8.1). Font programs and content streams therefore ship uncompressed, so Folio's PDFs are deliberately larger than a typical producer's. | Medium |
