...
> default. `SupportedVersion` rose to `3.1` for this change (`SupportedMajor` stays `3`; it is now `3.2`, for the unrelated `columns[].headerAlign`), so an existing document still
