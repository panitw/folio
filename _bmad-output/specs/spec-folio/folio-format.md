...
| `altRowBackground` | *Optional.* Colour for alternating rows (FR28). Alternation follows the row's index in the collection, so it does not reset per page. Colour-by-data is out of scope for this field exactly as it is for `style`'s own colour fields (see "Colours are `#RRGGBB`" below): a `{{ }}` placeholder here is a **load error** naming the element, under the same rejection. **Precedence, ruled at Story 4.2's creation and not yet implemented (Story 4.8 owns applying it):** `altRowBackground` wins over `style.background` on the rows it applies to — two colour sources on the same cell are ranked now, before 4.8 has to discover the conflict at the end of the gating epic. |
