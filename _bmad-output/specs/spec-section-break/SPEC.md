...
- **CAP-7**
  ...
  - **success:** Render the golden statement with an unanchored break 20pt above the legend. At 5 rows the legend is at its declared y. When the rows end 120pt below the line on page 1 and the legend fits in the room left, the legend is on page 1, 20pt below the end of the rows, and no row overlaps it. When it does not fit, it is on page 2 with the line at the top of the content window and the legend 20pt below it. Toggling Anchor in the Section Break Properties is one engine command and one undo entry, and it shows or hides the anchor icon in the canvas tab.
...
- Pulling an unanchored section up to close a gap when the content above ends short of the line.
