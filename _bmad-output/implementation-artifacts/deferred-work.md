...
**RESOLVED 2026-09-10 at `main`. Both causes measured, and my hypothesis was wrong.** I proposed that the pad's
`z-index: -1` sank it behind the band background. **Refuted by probe** (`document.elementsFromPoint` plus a
capturing `pointerdown` at the failing coordinate): the pad was reachable throughout and the
paint-outranks-pad arbitration worked as designed. Two *other* pieces of chrome were on top of it:

1. **The sticky toolbar.** `.canvas-region` declared `scroll-padding-top: 40px` - the bar's own height
   allowance - and forgot the region's `var(--space-10)` top padding sitting above it. `.canvas-tools` is
   `position: sticky; top: 0` inside that scroller and parks at 34..65 from the scrollport top, so content
   scrolled into view at 40 landed **underneath it**. Focusing the band scrolled it there, and the press hit
   the toolbar's zoom readout. Fixed by `calc(var(--space-10) + 40px)`.
2. **The component's own selection handles.** Since 14.3 a placed component *is* selected, so its handles draw
   immediately - `.selection-handle` is 12px on `-6px` insets and `.resize-handle` 24px on `-9px`, **reaching
   further outside the box than the hit pad does**. They swallowed the placement pointerup: the exact failure
   `.canvas-component-echo` and the pad's own suppression rule were each written against, arriving through a
   third piece of chrome nobody had suppressed. Fixed by extending the existing `.canvas-region-placing`
   suppression to the handles.

**Verified by the orchestrator, not relayed:** `npm run test:e2e` **49 passed, exit 0**; vitest 75/1285/0; tsc 0;
oxlint 0 with the 4 known warnings.

**The lesson is about the harness, not the CSS.** The `isolation: isolate` pair was ratified on a measurement
taken in a **throwaway Chromium harness** rather than the running app. The harness reproduced the stacking
question faithfully and reproduced **none of the surrounding chrome** - no sticky toolbar, no selection handles.
A reduced test case answers the question you put to it and is silent about the ones you did not think to ask,
which is exactly how a correct measurement can license a wrong conclusion.

### DW-367 - a test-regenerated evidence artifact is committed one request-id behind main

- **source_spec:** `_bmad-output/implementation-artifacts/evidence/story-6.7-roundtrip-manifest.json`
- **Found by:** the 14.3 hit-pad fix, whose tree it dirtied. **Owner:** unassigned. **Severity:** LOW. **Status:** OPEN.

The manifest is **regenerated by `e2e/image-asset.spec.ts` on every run**, and its committed copy differs from
what current `main` produces: `requestId` values shifted by one (`request-3` -> `request-4`). Some earlier
commit added one engine request ahead of that sequence and the evidence file was never refreshed.

**Proved not to be the fix's doing:** HEAD's `App.css` was temporarily restored, rebuilt, and that spec run
alone - the file produced was byte-identical to the one the fixed tree produced.
