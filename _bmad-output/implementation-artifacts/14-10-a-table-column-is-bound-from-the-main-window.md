...
**In plain terms.** Everything in a Folio template is bound the same way: you select the thing on the
canvas, open the DATA tab, and pick the field you want it to show. Everything except a table column.
To bind one of those you have to open the table editor — a separate dialog — and type the field name
into a text box. It is the one value in the product that is edited somewhere else, and it is the only
one you cannot simply point at.

This story makes a table column behave like everything else. Click a column on the canvas and it
becomes the selection; the DATA panel then offers exactly the fields that make sense for it — the
per-row fields of the collection the table is bound to — and picking one binds the column. Fields
that would not work for a column are not offered, and the panel says why rather than letting you pick
something the engine will reject a moment later. It undoes in one step, like every other binding.

The table editor keeps *showing* each column's bound field, because that is useful context while you
are arranging columns. What it stops doing is letting you *edit* it there. That removal is the point
rather than a side effect: two places to change one value is the problem this story exists to end,
and the editor's own design mockup never drew that text box in the first place — it drew a plain
label. The editor goes back to being about structure: adding, removing, reordering, widths, alignment
and footer totals.

**What this story does not do.** A column selected this way can be reached with a mouse and by no
other means — not by keyboard. That is a stated obligation the product has not met, left unpaid by an
explicit owner decision and recorded rather than glossed over; the requirement is quoted in full
below. This story also does not touch what the canvas *draws* for a table, which Story 14.9 settled,
and it adds no new capability to the engine — the information needed to identify a column was already
being sent to the browser; nothing was reading it.
